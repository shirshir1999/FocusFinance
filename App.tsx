...
import { supabase, saveUserData, fetchUserData, deleteUserData, hasMissingKeys, clearCustomKeys } from './services/supabase';
...
const App: React.FC = () => {
  ...
  const prevActiveView = useRef<TabId>(activeView);
  ...
  // Scroll to top when view changes - FIXED
  useEffect(() => {
      // Only scroll if the view ACTUALLY changed. This prevents jumps on data refresh.
      if (prevActiveView.current !== activeView && mainScrollRef.current) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      // Only update if the user actually changed to avoid unnecessary re-renders/loading triggers
      // Note: session.user object reference changes on token refresh, so we must be careful in effects
      setCurrentUser(session?.user ?? null);
  ...
  const loadPortfolioData = async (userId: string) => {
      ...
      // Only reset view if we are essentially "booting up" or switching users significantly
      ...
      try {
          ...
          if (cloudData) {
              ...
              // We need to fetch the user metadata again to be sure if currentUser is stale in closure
              const { data: { user } } = await supabase.auth.getUser(); 
              const name = user?.user_metadata?.full_name || 'פרופיל ראשי';
  ...
  useEffect(() => {
    // Only trigger load if userId changes. This prevents re-loading on token refreshes.
    if (currentUser?.id) {
  ...
  useEffect(() => {
    ...
    if (currentUser && isDataLoaded && data !== initialData) {
      ...
      saveUserData(currentUser.id, dataToSave);
    }
  ...
  const getMainUserName = () => {
      ...
      // 2. Try to get Main User name from the loaded data
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full relative">
          <header className="bg-white border-b border-slate-200 z-30 shadow-sm flex-shrink-0">
              <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                  ...
                  {/* Logo Area */}
                  <div className="flex items-center gap-2 md:gap-3 cursor-pointer min-w-0 ml-auto lg:ml-0" onClick={() => setActiveView('dashboard')}>
                  ...
                  <div className="flex items-center gap-2 md:gap-4 shrink-0">
                      ...
                      <div className="relative">
                          <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition bg-slate-100 text-slate-700 hover:bg-slate-200`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col overscroll-none" ref={mainScrollRef}>
